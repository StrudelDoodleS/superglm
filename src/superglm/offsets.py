...
A normal NumPy offset preserves only row values.  The classes in this module
also preserve the expression needed to reproduce the offset in a deployment
runtime such as SQL.
"""
...
class LogRatioOffset:
    ...
    With a log link this contributes the response-scale multiplier
    ``(source_feature / reference) ** coefficient``.  The coefficient is fixed;
    it is not fitted and therefore has no standard error or p-value.
    ...
    reference : float
        ...
        months, ``reference=12`` produces an annual-term multiplier.
    coefficient : float
    ...
    def to_record(self, *, sequence_no: int) -> dict[str, str | float | int]:
        """Return the stable workbook/deployment representation."""
        return {
            "Term": self.name,
...
def resolve_offset_input(
    ...
    """Resolve a structured offset to values while retaining deployment metadata."""
...
def fixed_offset_frame(model: Any) -> pd.DataFrame:
    ...
    rows = [term.to_record(sequence_no=i) for i, term in enumerate(deployable_offsets(model), 1)]
    return pd.DataFrame(rows, columns=columns)


class FixedOffsetModelSummary:
    """ModelSummary proxy that displays fixed offsets outside fitted coefficients."""
    ...
    def _records(self) -> list[dict[str, str | float | int]]:
        return [term.to_record(sequence_no=i) for i, term in enumerate(self._terms, 1)]
    ...
    def _repr_html_(self) -> str:
        ...
        for term in self._terms:
            rows.append(
                ...
                f"<td style='padding:3px 8px;text-align:left'>{escape(term.response_multiplier_expression)}</td>"
...
def install_deployable_offset_support(model_cls: type) -> None:
    """Install structured-offset fit, prediction, and summary support once."""
