...
class FactorSmoothResult:
    """Shared smoothing parameters, level credibility, and fitted curves."""
...
def _factor_penalties(
    ...
    matching = [
        ...
        if component.group_name == group.name and component.penalty_kind == "repeated"
    ]
    if not matching:
        raise RuntimeError(f"FactorSmooth term {group.name!r} has no repeated REML components.")
    return matching
...
def factor_smooth_result(
    ...
    *,
    ...
    penalties = _factor_penalties(model, group)
    ...
    collapsed = bool(at_upper_boundary and all(at_upper_boundary.values()))
    if collapsed:
        warnings.warn(
    ...
    if active_group.size != n_levels * spec.k:
    ...
    information_rank = np.empty(n_levels, dtype=np.int64)
    for level_index in range(n_levels):
        information = support.information[level_index]
        information_rank[level_index] = np.linalg.matrix_rank(information)
        try:
    ...
    has_information = (np.asarray(support.count) > 0) & np.isfinite(
        np.trace(support.information, axis1=1, axis2=2)
    )
