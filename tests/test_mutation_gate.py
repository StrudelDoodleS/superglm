...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("  ImportError: leading whitespace still counts", True),
    ],
)
def test_is_symbol_error_separates_absence_from_misbehaviour(message, expected):
    assert mutation_gate.is_symbol_error(message) is expected
...
IMPORT = "from superglm.links import _LOG_LINK_ETA_MIN\n"
PARAM = "import pytest\n\n" + IMPORT
CASE = "tests/test_gate_case.py"
LINKS = "src/superglm/links.py"
...
def clone(tmp_path_factory):
    ...
    write(
        dest,
        CASE,
        IMPORT + "\n\ndef test_pin():\n    assert isinstance(_LOG_LINK_ETA_MIN, float)\n",
    )
...
def bump(repo: Path) -> None:
    ...
    assert "_LOG_LINK_ETA_MIN = -80.0" in source
    target.write_text(source.replace("_LOG_LINK_ETA_MIN = -80.0", "_LOG_LINK_ETA_MIN = -70.0", 1))
...
def run_gate(repo: Path, base: str, head: str = "HEAD", **extra: str) -> tuple[int, str]:
    proc = subprocess.run(
        ...
        text=True,
        check=False,
        env={"PATH": "/usr/bin:/bin", "HOME": str(repo), **extra},
    )
...
def test_parametrised_test_is_judged_on_its_items(clone):
    ...
    write(
        ...
        "def test_param(bound):\n    assert _LOG_LINK_ETA_MIN == bound\n",
    )
    commit(clone, "param")
    assert run_gate(clone, "gate-base") == (0, "PASS")
...
def test_added_test_red_at_head_is_a_defect_in_the_change(clone):
    ...
    write(
        clone,
        CASE,
        IMPORT + "\n\ndef test_always_fails():\n    assert _LOG_LINK_ETA_MIN == 12345\n",
    )
    commit(clone, "broken")
    assert run_gate(clone, "gate-base") == (1, "INCONCLUSIVE")
