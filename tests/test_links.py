...
def test_sqrt_stabilization_preserves_signed_zero_and_tiny_predictors():
    link = SqrtLink()
    eta = np.array([-1.0e-150, -0.0, 0.0, 1.0e-150])
    stabilized = stabilize_eta(eta, link)

    np.testing.assert_array_equal(stabilized, eta)
    assert np.signbit(stabilized[1])
    assert not np.signbit(stabilized[2])
    assert np.all(np.isfinite(link.deriv_inverse(stabilized)))


def test_sqrt_stabilization_round_trips_representable_tiny_means():
    link = SqrtLink()
    means = np.array([np.nextafter(0.0, 1.0), 1.0e-300, 1.0e-30, 1.0e-16])
