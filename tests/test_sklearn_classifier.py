...
class TestClassifierContract:
    ...
    def test_get_params(self, binary_data):
        clf = SuperGLMClassifier(selection_penalty=0.1, spline_penalty=0.5)
        params = clf.get_params()
        assert params["selection_penalty"] == 0.1
        assert params["spline_penalty"] == 0.5

    def test_set_params(self, binary_data):
        clf = SuperGLMClassifier()
        clf.set_params(selection_penalty=0.05)
        assert clf.selection_penalty == 0.05
...
class TestClassifierThreshold:
    def test_custom_threshold(self, binary_data):
        X, y = binary_data
        clf = SuperGLMClassifier(selection_penalty=0, threshold=0.8)
        clf.fit(X, y)
        preds = clf.predict(X)
...
class TestClassifierNdarrayInput:
    ...
    def test_ndarray_with_feature_names(self, binary_data):
        X, y = binary_data
        X_arr = X[["x1"]].values
        clf = SuperGLMClassifier(selection_penalty=0, feature_names=["signal"])
        clf.fit(X_arr, y)
