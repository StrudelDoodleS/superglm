...
def test_the_paragraph_carries_no_per_pair_timings():
    ...
    """
    paragraph = DOC.split("``max_cells`` bounds allocation AND time", 1)[1]
    paragraph = paragraph.split("A categorical margin never bins", 1)[0]
...
def test_a_too_wide_spline_cat_pair_is_retried_not_refused():
    ...
    # crosses the 1357 cap at 106 levels: 13 * 104 = 1352, 13 * 105 = 1365.
    assert "``L = 106`` levels" in DOC
