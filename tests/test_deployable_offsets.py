...
def test_log_ratio_offset_is_reused_for_prediction():
    ...
    ratio = automatic[X["term"].to_numpy() == 36].mean() / automatic[
        X["term"].to_numpy() == 12
    ].mean()
    assert ratio > 2.0
...
def test_raw_array_offset_does_not_claim_a_deployable_expression(tmp_path):
    rng = np.random.default_rng(9)
    X = pd.DataFrame({"score": rng.normal(size=80), "term": rng.choice([12.0, 36.0], 80)})
