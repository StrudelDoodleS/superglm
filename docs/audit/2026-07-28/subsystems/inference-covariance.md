...
18. **Two conditional/frequentist conventions live only in comments.** SEs are consistently documented as Bayesian/conditional (`coefficient_se` note, metrics.py:986–989: "conditional-on-the-selected-model ... same convention as the reference lasso implementation / the reference implementation"); there is no Ve (frequentist sandwich) or selection-corrected covariance anywhere, and no flag distinguishing Vb vs Ve in APIs — the reference implementation users may expect `unconditional=TRUE`-style options. Not a bug; an architecture gap to note for the target-architecture phase.
