...
- [ ] **deploy.md example**: `model.export_rating_tables(<temp dir>)` (inside a `tempfile.TemporaryDirectory()`), list the files written and show the head of one table read back with pandas; or `rating_table_payload()` if the export needs arguments the simulated model cannot satisfy. Say which.
