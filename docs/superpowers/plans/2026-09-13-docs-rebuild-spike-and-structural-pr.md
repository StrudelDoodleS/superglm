...
**Interfaces:**
- Produces: the `{autosummary}` block shape used by every API page in Task 9; a count of numpydoc warnings on real docstrings.
...
2. `py:class reference target not found: superglm.<module>.<Name>` for real objects that have no page yet: these resolve once Task 9 adds every public name; leave them.
...
The reference is generated by autosummary from the ``{autosummary}`` blocks
under ``docs/api``. A new export without an entry fails here, not on a
reader's screen.
"""
...
AUTOSUMMARY_BLOCK = re.compile(r"```\{autosummary\}(.*?)```", re.S)
...
Each page is one orientation paragraph followed by an `{autosummary}` block. The name lists below are complete; together with `model.md` they cover all 131 public names. Write each file exactly.
