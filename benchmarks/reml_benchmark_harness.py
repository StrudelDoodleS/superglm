...
def run_mtpl2_benchmarks():
    ...
    # Basis size matching with the reference implementation:
    # the reference implementation bs="cr" k=K gives K basis functions (K-1 after identifiability).
    # SuperGLM CRS n_knots=N gives N+4 B-splines, N+2 after natural constraints
    # (intercept handled separately). To match the reference implementation k=20: n_knots=18 → 20 cols.
    ...
    for discrete in [False, True]:
        ...
        r = run_one(
            ...
            exposure=exposure,
            features={
                "DrivAge": CubicRegressionSpline(
                    n_knots=18
                ),  # matches the reference implementation k=20
                "VehAge": CubicRegressionSpline(
                    n_knots=13
                ),  # matches the reference implementation k=15
                "BonusMalus": CubicRegressionSpline(
                    n_knots=13
                ),  # matches the reference implementation k=15
